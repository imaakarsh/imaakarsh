<div align="center">
  <a href="https://postimg.cc/pytCmbKy">
    <img src="https://i.postimg.cc/T2K4HTwj/2ede7b2dc3b86f162d05817aed376b73.jpg" width="100%" alt="Banner"/>
  </a>
</div>

<br>

<div align="center"> 

# Aakarsh Raj

#### Full-Stack Developer • MERN Stack • B.Tech CSE @ Graphic Era University

[Portfolio](https://aakarshdev.me) •
[GitHub](https://github.com/imaakarsh) •
[LinkedIn](https://linkedin.com/in/aakarshraj07) •
[Twitter](https://twitter.com/imaakarshh) •
[Email](mailto:aakarsh027@outlook.com)

</div>

---

I'm a **B.Tech CSE student** passionate about building modern, responsive, and scalable web applications.

Currently focused on the **MERN Stack**, improving my **DSA & C++** skills, and building real-world projects while learning in public.

---

## selected work

- **FocusFlow** — Productivity web app with Pomodoro timer, task management, and clean UI. [Live](https://focusflow247.vercel.app/)
- **MindMate** — AI-powered study planner that adapts schedules based on mood and stress.
- **Portfolio Website** — Personal portfolio showcasing projects, skills, and experience. [Live](https://aakarshdev.me)
- **Blog Website** — Responsive blog platform built using HTML, CSS, and JavaScript.

---



## currently

- Building full-stack MERN applications
- Learning React & Next.js
- Solving DSA problems in C++
- Exploring Open Source
- Looking for internship opportunities

---



<div align="center">
  
# 📊 GitHub Stats:
![](https://github-readme-stats.shion.dev/api?username=imaakarsh&theme=dark&hide_border=false&include_all_commits=true&count_private=false)<br/>
![](https://streak-stats.demolab.com/?user=imaakarsh&theme=dark&hide_border=false)<br/>
![](https://github-readme-stats.shion.dev/api/top-langs/?username=imaakarsh&theme=dark&hide_border=false&include_all_commits=true&count_private=false&layout=compact)

---
[![](https://komarev.com/ghpvc/?username=imaakarsh&icon=0&color=0)](https://visitcount.itsvg.in)

<!-- Proudly created with GPRM ( https://gprm.itsvg.in ) -->
</div>


## connect

- 🌐 Portfolio → [https://aakarshdev.me](https://aakarshdev.me)
- 💼 LinkedIn → [https://linkedin.com/in/](https://www.linkedin.com/in/aakarshraj07/)
- 📧 Email → aakarsh027@outlook.com

---

<div align="center">

*"Learning in public. Building one project at a time."*

</div>

<div align="center">
  <a href="https://postimg.cc/XBBCDcJP">
    <img src="https://i.postimg.cc/prqJrstP/20260612-025300.jpg" width="100%" alt="Coding Quotes"/>
  </a>
</div>

